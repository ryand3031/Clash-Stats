...
def get_player_info(pid):
    r=requests.get( f"https://api.clashroyale.com/v1/players/%23{pid}", headers={"Accept":"application/json", "authorization":f"Bearer {key}"}, params = {"limit":20})
    #print(json.dumps(r.json(), indent = 2))
    return r.json()

def get_player_chests(pid):
    r=requests.get( f"https://api.clashroyale.com/v1/players/%23{pid}/upcomingchests", headers={"Accept":"application/json", "authorization":f"Bearer {key}"}, params = {"limit":20})
    return r.json()
